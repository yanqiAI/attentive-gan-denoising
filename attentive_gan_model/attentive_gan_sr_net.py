...
class GenerativeNet(cnn_basenet.CNNBaseModel):
    ...
    def build_autoencoder(self, input_tensor, name, reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            ### add one deconvolution layer (resolution * 2)
            deconv_3 = self.deconv2d(inputdata=relu_16, out_channel=32, kernel_size=4,
                                     stride=2, padding='SAME', use_bias=False, name='deconv_3')
            avg_pool_3 = self.avgpooling(inputdata=deconv_3, kernel_size=2, stride=1, padding='SAME',
                                         name='avg_pool_3')
            relu_17 = self.lrelu(inputdata=avg_pool_3, name='relu_17')

            # 去掉残差连接，直接卷积+lrelu
            conv_11 = self.conv2d(inputdata=relu_17, out_channel=16, kernel_size=3,
                                  padding='SAME', stride=1, use_bias=False,
                                  name='conv_11')
            relu_18 = self.lrelu(inputdata=conv_11, name='relu_18')

            # conv_10 = self.conv2d(inputdata=tf.add(relu_17, relu_1), out_channel=32, kernel_size=3,
            #                       padding='SAME', stride=1, use_bias=False,
            #                       name='conv_10')
            # relu_16 = self.lrelu(inputdata=conv_10, name='relu_16')

            # skip_output  modify the output size
            skip_output_1 = self.conv2d(inputdata=relu_14, out_channel=1, kernel_size=3,
                                        padding='SAME', stride=1, use_bias=False,
                                        name='skip_ouput_1')

            skip_output_2 = self.conv2d(inputdata=relu_16, out_channel=1, kernel_size=3,
                                        padding='SAME', stride=1, use_bias=False,
                                        name='skip_output_2')

            skip_output_3 = self.conv2d(inputdata=relu_18, out_channel=1, kernel_size=3,
                                        padding='SAME', stride=1, use_bias=False,
                                        name='skip_output_3')
