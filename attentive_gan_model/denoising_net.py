...
# from attentive_gan_model import attentive_gan_sr_net
# from attentive_gan_model import discriminative_sr_net
...
class DenoisingNet(object):
    ...
    def __init__(self, phase):
        ...
        # self._attentive_gan = attentive_gan_sr_net.GenerativeNet(self._phase)
        # self._discriminator = discriminative_sr_net.DiscriminativeNet(self._phase)

    def compute_loss(self, input_tensor, gt_label_tensor, mask_label_tensor, name, reuse=False):
        """
        计算gan损失和discriminative损失
        :param input_tensor: 含有噪声的图像
        :param gt_label_tensor: 不含有噪声的图像
...
if __name__ == '__main__':
    ...
    net = DenoisingNet(tf.constant('train', tf.string))
