...
def init_args():
    ...
    parser.add_argument('--dataset_dir', type=str, default='E:\datasets\denoising\processing_data', help='The source nsfw data dir path')
    parser.add_argument('--tfrecords_dir', type=str, default='E:\datasets\denoising\processing_data', help='The dir path to save converted tfrecords')

    return parser.parse_args()


class DenoisingDataProducer(object):
    """
    ...
    def __init__(self, dataset_dir):
        ...
        self._blur_image_dir = ops.join(dataset_dir, 'dirty_train_0509') #256*256
        self._clean_image_dir = ops.join(dataset_dir, 'clean_train_0509') #256*256
    ...
    def generate_tfrecords(self, save_dir, step_size=10000):
        ...
        train_blur_images_paths, train_clean_images_paths = _read_training_example_index_file(
            self._train_example_index_file_path)
        train_blur_images_paths_split, train_clean_images_paths_split, train_tfrecords_save_paths = \
            _split_writing_tfrecords_task(train_blur_images_paths, train_clean_images_paths, _flags='train')

        for index, example_paths in enumerate(train_blur_images_paths_split):
        ...
        val_blur_images_paths, val_clean_images_paths = _read_training_example_index_file(
            self._val_example_index_file_path)
        val_blur_images_paths_split, val_clean_images_paths_split, val_tfrecords_save_paths = \
            _split_writing_tfrecords_task(val_blur_images_paths, val_clean_images_paths, _flags='val')

        for index, example_paths in enumerate(val_blur_images_paths_split):
            tf_io_pipline_tools.write_example_tfrecords(example_paths,
        ...
        test_blur_images_paths, test_clean_images_paths = _read_training_example_index_file(
            self._test_example_index_file_path)
        test_blur_images_paths_split, test_clean_images_paths_split, test_tfrecords_save_paths = \
            _split_writing_tfrecords_task(test_blur_images_paths, test_clean_images_paths, _flags='test')

        for index, example_paths in enumerate(test_blur_images_paths_split):
            tf_io_pipline_tools.write_example_tfrecords(example_paths,
    ...
    def _is_source_data_complete(self):
        ...
        :return:
        """
        return ops.exists(self._blur_image_dir) and ops.exists(self._clean_image_dir)
    ...
    def _generate_training_example_index_file(self):
        ...
        def _gather_example_info():
            ...
            """
            _info = []
            for _blur_image_path in glob.glob('{:s}/*.jpg'.format(self._blur_image_dir)):
                _clean_image_name = ops.split(_blur_image_path)[1]
                ...
                _info.append('{:s} {:s}\n'.format(_blur_image_path, _clean_image_path))
...
class DenoisingDataFeeder(object):
    """
    ...
    def __init__(self, dataset_dir, flags='train'):
        ...
        self._tfrecords_dir = ops.join(dataset_dir, 'tfrecords_0509')
...
if __name__ == '__main__':
    ...
    producer = DenoisingDataProducer(dataset_dir=args.dataset_dir)
    producer.generate_tfrecords(save_dir=args.tfrecords_dir, step_size=50000)
