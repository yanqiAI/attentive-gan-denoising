...
def init_args():
    ...
    parser.add_argument('--dataset_dir', type=str, default='E:/datasets/denoising/processing_data', help='The dataset dir')
    parser.add_argument('--weights_path', type=str, help='The pretrained weights path', default=None)
...
def train_model(dataset_dir, weights_path=None):
    ...
    with tf.device('/cpu:0'):
        ...
        train_dataset = data_feed_pipline.DenoisingDataFeeder(dataset_dir=dataset_dir, flags='train')
        val_dataset = data_feed_pipline.DenoisingDataFeeder(dataset_dir=dataset_dir, flags='val')
    ...
    with tf.device('/gpu:1'):
        # define network
        denoise_net = denoising_net.DenoisingNet(phase=tf.constant('train', dtype=tf.string))

        # calculate train loss and validation loss
        train_gan_loss, train_discriminative_loss, train_net_output = denoise_net.compute_loss(
        ...
        val_gan_loss, val_discriminative_loss, val_net_output = denoise_net.compute_loss(
        ...
        model_save_dir = 'model/denoising_gan'
        if not ops.exists(model_save_dir):
            os.makedirs(model_save_dir)
        train_start_time = time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime(time.time()))
        model_name = 'denoising_gan_{:s}.ckpt'.format(str(train_start_time))
        ...
        with sess.as_default():
            ...
            tf.train.write_graph(graph_or_graph_def=sess.graph, logdir='',
                                 name='{:s}/denoising_gan.pb'.format(model_save_dir))
            ...
            # 加载预训练参数，需要先下载vgg16.npy文件，放在vgg_model下面
